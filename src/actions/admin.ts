...
export const deleteUserPredictionAsAdminAction = async (
  targetUserId: string,
): Promise<{ success: true; data: null } | { success: false; error: string }> => {
  try {
    ...
    const { deleted } = await deleteUserPredictionForAdmin(user.id, parsed.data);
    if (!deleted) {
      return { success: false, error: 'No hay pronóstico para este usuario.' };
    }
