-- Diagnóstico: Rep. de Corea 2–1 Chequia (match_number = 2) vs puntos asignados.
...
WITH kor_cze AS (
  ...
  WHERE match_number = 2
    AND home_team_id = 'kor'
    AND away_team_id = 'cze'
  LIMIT 1
),
preds AS (
  SELECT
  ...
  LEFT JOIN public.prediction_matches pm
   ...
   AND pm.match_id = (SELECT id FROM kor_cze)
  LEFT JOIN kor_cze m ON true
