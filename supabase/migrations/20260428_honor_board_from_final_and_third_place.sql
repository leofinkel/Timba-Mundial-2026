-- =============================================================================
-- Cuadro de honor: subcampeón = perdedor predicho en la final (partido 104);
-- 4.º puesto = perdedor predicho en el 3.º/4.º puesto (partido 103).
-- Alinea calculate_user_score con scoringService.ts (real_results vs predicción).
-- =============================================================================
...
BEGIN
  ...
  IF v_real IS NOT NULL THEN
    ...
    -- Runner-up: the other finalist in the final match the user did not pick (loser of final in prediction)
    IF v_real.runner_up_team_id IS NOT NULL THEN
      ...
      FROM public.prediction_matches pm
      JOIN public.matches m ON m.id = pm.match_id
      WHERE pm.prediction_id = v_prediction_id AND m.stage = 'final'
      LIMIT 1;
