-- Goleador / figura: aceptar "Nombre Apellido" o solo "Apellido" en cualquiera de
-- los dos lados, sin distinguir acentos ni mayusculas/minusculas.

CREATE OR REPLACE FUNCTION public.normalize_special_prediction_name(p_name TEXT)
RETURNS TEXT LANGUAGE sql IMMUTABLE AS $$
  SELECT trim(
    regexp_replace(
      translate(
        lower(coalesce(p_name, '')),
        'áàâãäåéèêëíìîïóòôõöúùûüñçýÿ',
...
CREATE OR REPLACE FUNCTION public.special_prediction_names_match(p_a TEXT, p_b TEXT)
RETURNS BOOLEAN LANGUAGE sql IMMUTABLE AS $$
  SELECT
    a <> '' AND b <> '' AND (
      a = b
      OR reverse(split_part(reverse(a), ' ', 1)) = reverse(split_part(reverse(b), ' ', 1))
    )
  FROM (
    SELECT
      public.normalize_special_prediction_name(p_a) AS a,
      public.normalize_special_prediction_name(p_b) AS b
  ) n
$$;
...
COMMENT ON FUNCTION public.calculate_user_score(UUID) IS
  'Read-only w.r.t. predictions: reads prediction_* vs official matches/real_results, upserts only user_scores. Honor 103/104 uses pred team slots; goleador/figura acepta nombre completo o solo apellido, sin acentos/mayusculas.';
