-- Subcampeon: otorgar los 100 puntos siempre que el 2do puesto predicho coincida
-- con el subcampeon real, sin importar si tambien acerto el campeon. La version
-- anterior excluia el bono de subcampeon cuando el usuario acertaba el campeon
-- (ver 20260431_honor_runner_exclusive_goals_first_clear_r16.sql), lo cual no
-- coincide con la logica de scoringService.ts / computeUserScore.ts (TS), donde
-- campeon y subcampeon siempre se evaluan de forma independiente.
...
BEGIN
  ...
  IF v_official_champ IS NOT NULL THEN
    SELECT
      CASE
        WHEN pm.pred_home_team_id IS NULL OR pm.pred_away_team_id IS NULL THEN NULL
        WHEN pm.home_goals > pm.away_goals THEN pm.pred_home_team_id
        WHEN pm.away_goals > pm.home_goals THEN pm.pred_away_team_id
        WHEN pm.home_goals = pm.away_goals
          AND pm.winner_team_id IN (pm.pred_home_team_id, pm.pred_away_team_id) THEN pm.winner_team_id
        ELSE NULL
      END
  ...
  -- Runner-up: perdedor predicho (2do puesto del bracket final) = subcampeon real.
  -- Independiente de si tambien acerto el campeon (igual que TS: ambos honores se
  -- evaluan por separado).
    ...
    WITH pw AS (
      SELECT
        CASE
          ...
          ELSE NULL
        END AS w,
        pm.pred_home_team_id AS h,
        pm.pred_away_team_id AS a
      FROM public.prediction_matches pm
      JOIN public.matches m ON m.id = pm.match_id
      WHERE pm.prediction_id = v_prediction_id
        AND (m.match_number = 104 OR m.stage = 'final')
  ...
  IF v_real IS NOT NULL THEN
    -- Top scorer: acepta nombre completo o solo apellido; sin acentos ni mayusculas
    IF v_real.top_scorer IS NOT NULL AND v_pred_special IS NOT NULL
       AND public.special_prediction_names_match(v_pred_special.top_scorer, v_real.top_scorer)
    THEN v_scorer_pts := 100; END IF;

    -- Best player: misma normalizacion que top scorer
    IF v_real.best_player IS NOT NULL AND v_pred_special IS NOT NULL
       AND public.special_prediction_names_match(v_pred_special.best_player, v_real.best_player)
...
COMMENT ON FUNCTION public.calculate_user_score(UUID) IS
  'Read-only w.r.t. predictions: reads prediction_* vs official matches/real_results, upserts only user_scores. Honor 103/104 uses pred team slots; goleador/figura acepta nombre completo o solo apellido, sin acentos/mayusculas; subcampeon independiente del campeon.';
